...
class CONTROLLER():
    ...
    def __init__(self):
        ...
        self.motor, self.serial_port=sd.init()  #stepper_drive
        ms.init()
        ms.motor_on()
        self.motoron = True
        self.servo_angle=0
        ...
        self.set_max_speed_camera_x(80000)
        self.set_max_speed_camera_z(80000)
        self.set_max_speed_mirror_z(80000)
        self.set_max_speed_mirror_height(80000)
        #initalize maximum acceleration
        self.acceleration = []
        for i in range (0,6):
            self.acceleration.append(self.motor[i].get_axis_parameter(5))
        self.set_acceleration_camera_x(13000)
        self.set_acceleration_camera_z(13000)
        self.set_acceleration_mirror_z(13000)
        self.set_acceleration_mirror_height(13000)
        #initalize current  
        self.current = [] 
        for i in range (0, 6):
        ...
        if "who_am_i" in this_config:
            ...
            self.set_servo_angle(int(this_config["current_state"]["servo_angle"]))
        else:
        ...
        logging.info("Initalized controller with gear ratio {}".format(self.gear_ratio_mirror_height))
    ...
    def set_acceleration_camera_z(self, value, verbose=False):
    ...
    def set_acceleration_camera_x(self, value, verbose=False):
    ...
    def set_acceleration_mirror_z(self, value, verbose=False):
    ...
    def set_acceleration_mirror_height(self, value, verbose=False):
    ...
    def set_acceleration_mirror_psi(self, value, verbose=False):
    ...
    def set_acceleration_mirror_phi(self, value, verbose=False):
    ...
    def get_acceleration_camera_z(self):
        return self.acceleration[0]
    def get_acceleration_camera_x(self):
        return self.acceleration[1]
    def get_acceleration_mirror_z(self):
        return self.acceleration[2]
    def get_acceleration_mirror_height(self):
        return self.acceleration[3]
    def get_acceleration_mirror_psi(self):
        return self.acceleration[4]
    def get_acceleration_mirror_phi(self):
    ...
    def set_position_camera_z(self, position, verbose=False):
        ...
        if self.log_on_set_position: logging.info("CAM Z: {0:4.2f}".format(position))
    ...
    def set_position_camera_x(self, position, verbose=False):
        ...
        if self.log_on_set_position: logging.info("CAM X: {0:4.2f}".format(position))
    ...
    def set_position_mirror_z(self, position, verbose=False):
        ...
        if self.log_on_set_position: logging.info("MIR Z: {0:4.2f}".format(position))
    ...
    def set_position_mirror_height(self, position, verbose=False):
        ...
        if self.log_on_set_position: logging.info("MIR Y: {0:4.2f}".format(position))
    ...
    def set_position_mirror_psi(self, position, verbose=False):
        ...
        if self.log_on_set_position: logging.info("MIR PSI: {0:4.2f}".format(position))
    ...
    def set_position_mirror_phi(self, position, verbose=False):
        ...
        if self.log_on_set_position: logging.info("MIR PHI: {0:4.2f}".format(position))
    ...
    def get_position_mirror_phi(self):
        pos=self.steps_to_degree(sd.position(self.motor[5]))
        if pos>4.5 or pos<-4.5:
            return 0
        else:
            return pos
    ...
    def set_servo_angle(self, value):
        ...
        #write the angle of the servo into the this_pc.conf file
        this_config = configparser.ConfigParser()
        path='../../../this_pc.conf'
        this_config.read(path)
        if "who_am_i" in this_config:
            if this_config["who_am_i"]["type"]!="motor_pc":
                print("According to the 'this_pc.config'-file this pc is not meant as a motor pc! Please fix that!")
                exit()
            this_config["current_state"]["servo_angle"]=str(self.servo_angle)
            this_config.write(open(path,'w'))
        else:
            print("There is no config file on this computer which specifies the computer function! Please fix that!")
            exit()
        servo.shutter(value)
