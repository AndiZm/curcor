...
class GUI:
    ...
    def __init__(self, master, controller, position=None, client=None, log=None):
        ...
        if self.controller.get_motor_on():
            ...
            onoffButton.config(text="OFF", bg="#cc3960", padx=3, pady=3)
    ...
    def psupp_connect(self):
        try:
            ...
            if self.controller.halogen.on == 1:
                ...
                self.psupp_onoffButton.config(bg="#91CC66")
            else:
                self.psupp_onoffButton.config(text="Off")
                self.psupp_onoffButton.config(bg="#cc3960")
    ...
    def go_to_parking_position(self):
        self.CameraZ.set(0)
        self.CameraX.set(0)
        self.MirrorZ.set(500) #more extreme so it gets to the max for sure!
        self.MirrorHeight.set(0) #more extreme so it gets to the max for sure!
        self.MirrorPsi.set(4.5)
        self.MirrorPhi.set(0)
        self.change_position_camera_z=True
        self.change_position_camera_x=True
    ...
    def update_items(self, verbose=False):
        ...
        if self.change_acceleration_camera_x:
            self.controller.set_acceleration_camera_x(self.CameraXAcc.get(), verbose)
            self.change_acceleration_camera_x=False  
        if self.change_acceleration_mirror_z:
            self.controller.set_acceleration_mirror_z(self.MirrorZAcc.get(), verbose)
            self.change_acceleration_mirror_z=False
        if self.change_acceleration_mirror_height:
            self.controller.set_acceleration_mirror_height(self.MirrorHeightAcc.get(), verbose)
            self.change_acceleration_mirror_height=False
        if self.change_acceleration_mirror_psi:
            self.controller.set_acceleration_mirror_psi(self.MirrorPsiAcc.get(), verbose)
