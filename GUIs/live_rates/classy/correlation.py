...
def autocorrelation(data_in):

    data = data_in
    data = data.reshape(int(len(data)/2),2)
    data_A = data[:,0]; data_B = data[:,1]

    # Get file creation time
    #ctime = file_time(this_pc1_filename, this_pc2_filename)

    iterations=int(len(data)/length)-1
    this_cor = cupy.zeros((acorlen))  

    # Do the correlation
    for i in range(iterations):
        # Auto correlation A X B
        data_B_cu = cupy.array(data_B[ int(corlen/2)+(length*i) : (-1)*int(corlen/2)+(length*(i+1)) ]).astype(np.float32)
        data_A_cu = cupy.array(data_A[length*i:length*(i+1)]).astype(np.float32)
        this_cor += cupy.correlate(data_B_cu, data_A_cu, "valid")
