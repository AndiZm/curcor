...
root = Tk(); root.wm_title("DAQ Control")
r_width  = 20; r_height = 500 # dimensions of the rate bars
...
fig = Figure(figsize=(3*len(tel_nos),0.5))
...
class telescope(object):
	...
	def __init__(self, tel_number, tel_name, spcm_port, datapath, port, mode):
		...
		self.optionLabel = Label(self.leftFrame, text="{}".format(tel_name)+"\n{0} sn {1:05d}".format(self.card.sCardName, self.card.lSerialNumber.value), font=("Helvetica 12 bold")); self.optionLabel.grid(row=2,column=0)

		## STATUS FRAME AND BUTTON ##
		self.statusFrame = Frame (self.leftFrame); self.statusFrame.grid(row=3, column=0)
		...
		# Sampling frequency
		self.binningFrame = Frame(self.coptionFrame); self.binningFrame.grid(row=2,column=0)
		...
		# Voltage range
		self.voltageFrame = Frame(self.coptionFrame); self.voltageFrame.grid(row=3,column=0)
		...
		# Channels
		self.channelFrame = Frame(self.coptionFrame); self.channelFrame.grid(row=4,column=0)
		...
		# Clockmode
		self.clockFrame = Frame(self.coptionFrame); self.clockFrame.grid(row=5,column=0)
		...
		self.wf_fig = Figure(figsize=(3.3,2.8))
		# Plot waveforms
		self.wf_a = []; self.wf_b = []
		...
		self.ledFrame = Frame(self.rateFrame); self.ledFrame.grid(row=0,column=0)
		self.waitCanvas = Canvas(self.ledFrame, width=30,height=30); self.waitCanvas.grid(row=0,column=0)
		self.waitLED = self.waitCanvas.create_rectangle(1,1,30,30, fill="black", width=0)

		self.rateBarFrame = Frame(self.rateFrame); self.rateBarFrame.grid(row=1,column=0)

		# This is the frame containing the small bars which display the rates relative to the maximum allowed rate		
		self.rateACanvas = Canvas(self.rateBarFrame, width=r_width, height=r_height, bg="gray"); self.rateACanvas.grid(row=0,column=0)
		self.rateBCanvas = Canvas(self.rateBarFrame, width=r_width, height=r_height, bg="gray"); self.rateBCanvas.grid(row=0,column=1)
		...
		self.offsetLEntry = Entry(self.offsetHeader, width=8); self.offsetLEntry.insert(0,"1000000")#; self.offsetLEntry.grid(row=0,column=2); 
		self.offsetPLabel = Label(self.offsetHeader, text="p", background="#e8fcae")#; self.offsetPLabel.grid(row=0,column=3)
		self.offsetPEntry = Entry(self.offsetHeader, width=5); self.offsetPEntry.insert(0,"2000")#; self.offsetPEntry.grid(row=0,column=4) 
		...
		self.parOffsetLabelA = Label(self.offsetParamFrame, text="{:.2f}".format(self.calibs.off_a), background="black", fg="orange"); self.parOffsetLabelA.grid(row=6,column=1, padx=10)
		...
		self.calibLEntry = Entry(self.calibHeader, width=8); self.calibLEntry.insert(0,"1000000")#; self.calibLEntry.grid(row=0,column=2); 
		self.calibPLabel = Label(self.calibHeader, text="p", background="#ccf2ff")#; self.calibPLabel.grid(row=0,column=3)
		self.calibPEntry = Entry(self.calibHeader, width=5); self.calibPEntry.insert(0,"200")#; self.calibPEntry.grid(row=0,column=4); 
		...
		self.fitRangelowEntryA  = Entry(self.calibParamFrame, width=5); self.fitRangelowEntryA.grid(row=1, column=1); self.fitRangelowEntryA.insert(0,"-40")
		self.fitRangelowEntryB  = Entry(self.calibParamFrame, width=5); self.fitRangelowEntryB.grid(row=1, column=2); self.fitRangelowEntryB.insert(0,"-40")
		...
		self.fitRangehighEntryA = Entry(self.calibParamFrame, width=5); self.fitRangehighEntryA.grid(row=2, column=1); self.fitRangehighEntryA.insert(0,"-3")
		self.fitRangehighEntryB = Entry(self.calibParamFrame, width=5); self.fitRangehighEntryB.grid(row=2, column=2); self.fitRangehighEntryB.insert(0,"-3")		
		...
		self.minHeightEntryA    = Entry(self.calibParamFrame, width=5); self.minHeightEntryA.grid(row=3,column=1); self.minHeightEntryA.insert(0,"-10")
		self.minHeightEntryB    = Entry(self.calibParamFrame, width=5); self.minHeightEntryB.grid(row=3,column=2); self.minHeightEntryB.insert(0,"-10")
		...
		self.startstopLEntry      = Entry(self.startstopHeader, width=8); self.startstopLEntry.insert(0,"1000000")#; self.startstopLEntry.grid(row=0,column=3); 
		self.startstopPLabel      = Label(self.startstopHeader, text="p")#; self.startstopPLabel.grid(row=0,column=4)
		self.startstopPEntry      = Entry(self.startstopHeader, width=5); self.startstopPEntry.insert(0,"1")#; self.startstopPEntry.grid(row=0,column=5) 
		## Param Frame
		...
		self.ampAEntry          = Entry(self.abFrame, width=5); self.ampAEntry.insert(0,"10")#; self.ampAEntry.grid(row=1, column=1)
		self.ampBEntry          = Entry(self.abFrame, width=5); self.ampBEntry.insert(0,"10")#; self.ampBEntry.grid(row=1, column=2)
