...
# For plotting
rates_a = []; plotFig = []; rate_a_plot = []; wav_a = []; rates_a_jkb = []; rates_a_jkb_plot = []

# Jigar's CNN
from live_nn_analysis import resnet50
...
def analyze_file(newest_file):
	...
	with open(newest_file, 'rb') as f:
		...
		for allpkt in range(0, int(startstopPEntry.get())):
			...
			a_np = -1.*a_np
			f1 = resnet50(a_np, path) # accecc initializer/class
			f2 = f1.load_model(a_np,path) # perform prediction using resnet, which will give avg.rate(MHz)
			rates_jkb.append(f2)
			del(a_np)
	...
	if plotting == True:
		rates_a.append(r_a)
		rates_a_jkb.append(r_a_jkb)
		wav_a = wv.execute_single(file=newest_file, length=1000)
		global rate_a_plot, wv_a_plot, rate_a_jkb_plot
		plotFigAxis.cla(); plotFigAxis.set_xlabel("File index"); plotFigAxis.set_ylabel("Rates [MHz]")
		rate_a_plot = plotFigAxis.plot(rates_a, "o--", color="blue", label="CI")
		rate_a_jkb_plot = plotFigAxis.plot(rates_a_jkb, "o--", color="orange", label="CNN")
		plotFigAxis.legend()
...
def switchplot():
	...
	if plotting == False:
		...
		rate_a_jkb_plot = plotFigAxis.plot(rates_a_jkb, "o--", color="blue")
...
def clearPlot():
	global rates_a, wav_a, rates_a_jkb
	rates_a = []; wav_a = []; rates_a_jkb = []
