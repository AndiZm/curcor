...
#	gl.g2_sig = []
#	length = len(np.loadtxt(gl.files_sig[0])[:,0])
#	G2 = np.zeros(length)
#	for i in range (0,len(gl.files_sig)):
#		curr_file = np.loadtxt(gl.files_sig[i])
#		for j in range (0, length):
#			G2[j] += curr_file[:,1][j]
#	avg_sig = np.mean(G2[100:])
#	for j in range (0,length):
#		gl.g2_sig.append(G2[j]/avg_sig)
#	# Correction factor
#	c = float(gl.corrSigEntry.get())
#	for j in range (0,length):
#		gl.g2_sig[j] = (gl.g2_sig[j]-1.)*c + 1.
...
#def correlate_ref():
#	print ("Correlate reference ...")
#	gl.g2_ref = []
#	length = len(np.loadtxt(gl.files_ref[0])[:,0])
#	G2 = np.zeros(length)
#	for i in range (0,len(gl.files_ref)):
#		curr_file = np.loadtxt(gl.files_ref[i])
#		for j in range (0, length):
#			G2[j] += curr_file[:,1][j]
#	avg_ref = np.mean(G2[100:])
#	for j in range (0,length):
#		gl.g2_ref.append(G2[j]/avg_ref)
#	# Correction factor
#	c = float(gl.corrRefEntry.get())
#	for j in range (0,length):
#		gl.g2_ref[j] = (gl.g2_ref[j]-1.)*c + 1.
#	#gl.g2_ref_plot = gl.corrAx.plot(gl.g2_ref, color="grey")
#def correlate_diff():
#	gl.g2_diff = []
#	for j in range (0, len(gl.g2_sig)):
#		gl.g2_diff.append(gl.g2_sig[j] - gl.g2_ref[j] + 1.)
...
def fft(binning):
	...
	formax = 0.
	if gl.boolSig == True:
		gl.g2_sig_fft = np.fft.fft(gl.g2_sig)
	...
	if gl.boolRef == True:
		gl.g2_ref_fft = np.fft.fft(gl.g2_ref)
	...
	if gl.boolSig == True and gl.boolRef == True:
		gl.g2_diff_fft = np.fft.fft(gl.g2_diff)
...
def get_cumulative_g2_sig(lowpass, binning):
	...
	for j in range (0, len(gl.G2_cum_sig)):
		g2.append( ((gl.G2_cum_sig[j]/avg)-1.)*corr_factor +1.)
	if lowpass == True:
...
def get_cumulative_g2_ref(lowpass, binning):
	...
	for j in range (0, len(gl.G2_cum_ref)):
		g2.append( ((gl.G2_cum_ref[j]/avg)-1.)*corr_factor +1.)
	if lowpass == True:
		g2 = butter_lowpass_filter(g2, binning)
	return g2
...
def single_RMS_sig(fileindex, binning):
	...
	if gl.boolLP == True:
		g2 = butter_lowpass_filter(g2, binning)
	gl.rmssin_sig.append(corr_factor *  np.std(g2[range_left:range_right]))
def single_RMS_ref(fileindex, binning):
	...
	if gl.boolLP == True:
		g2 = butter_lowpass_filter(g2, binning)
	gl.rmssin_ref.append(corr_factor *  np.std(g2[range_left:range_right]))
...
def single_RMS_sig_exp(fileindex, binning):
	...
	rate_a = gl.rates_a_sig[fileindex]; rate_b = gl. rates_b_sig[fileindex]
	rms = 1./np.sqrt(rate_a*rate_b*1e12*binning*T)
...
def cumulative_RMS_sig_exp(fileindex, binning):
	...
	samples = int(lines[4].split("\t")[1]); T = binning * samples
	rate_a = gl.rates_a_sig[fileindex]; rate_b = gl. rates_b_sig[fileindex]
	gl.N_e_sig += rate_a*rate_b*1e12*binning*T; rms = 1./np.sqrt(gl.N_e_sig)
...
def cumulative_RMS_ref_exp(fileindex, binning):
	...
	samples = int(lines[4].split("\t")[1]); T = binning * samples
	rate_a = gl.rates_a_ref[fileindex]; rate_b = gl. rates_b_ref[fileindex]
	gl.N_e_ref += rate_a*rate_b*1e12*binning*T; rms = 1./np.sqrt(gl.N_e_ref)
...
def experimental_correction_factors():
	if gl.boolSig == True:
		...
		corr_frac_new = corr_frac_old/avg
...
def fit_signal(binning):
	...
	if gl.boolSig == True:
		...
		med_start = np.mean([lborder,rborder])
		amp_start = np.max(gl.g2_sig[lborder:rborder+1]) -1.
		sig_start = 2.
		...
		gl.intValLabel.config(text="{:.2f} +/- {:.2f} ps".format(I,dI))
...
def fit_difference(binning):
	...
	if gl.boolSig == True and gl.boolRef == True:
		...
		rborder = int(gl.fitRangeRightEntry.get())
		med_start = np.mean([lborder,rborder])
		amp_start = np.max(gl.g2_diff[lborder:rborder+1]) -1.
		sig_start = 2.
		x_fit = np.arange(lborder,rborder+1,1)
