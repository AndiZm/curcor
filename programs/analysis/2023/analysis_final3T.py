...
combicolors[1,4] = "cyan"
combicolors[3,4] = "orange"
...
def par_fixing(star, telcombi):
    c1 = telcombi[0]
    c2 = telcombi[1]
    pairstring = "{}{}".format(c1,c2)

    # Read in the data g2 functions
    chAs    = np.loadtxt("g2_functions/{}/{}/chA.g2".format(star, pairstring))     
    chBs    = np.loadtxt("g2_functions/{}/{}/chB.g2".format(star, pairstring))      
    ...
    print("{} A 470nm mean: {:.2f} +/- {:.2f} ns \t sigma: {:.2f} +/- {:.2f} ns \t integral: {:.2f} +/- {:.2f} fs".format(pairstring,mu_A[c1][c2], perr[1],sigma_A[c1][c2],perr[2],1e6*integral,1e6*dintegral))
    plt.plot(x, g2_allA, label=pairstring + "A", color="green")
    ...
    integral, dintegral = uti.integral(popt, perr)
    print ("{} B 375nm mean: {:.2f} +/- {:.2f} ns \t sigma: {:.2f} +/- {:.2f} ns \t integral: {:.2f} +/- {:.2f} fs".format(pairstring,mu_B[c1][c2],perr[1],sigma_B[c1][c2],perr[2],1e6*integral,1e6*dintegral))
    plt.plot(x, g2_allB, label=pairstring + "B", color="blue")
    plt.plot(xplot, uti.gauss(xplot,*popt), color="black", linestyle="--")
...
plt.figure("blue"); plt.title("470 nm")
plt.xlabel("Baseline (m)")
plt.ylabel("Spatial coherence (fs)")
plt.xlim(0,170)
plt.axhline(y=0, color="grey", linestyle="--")

plt.figure("uv"); plt.title("375 nm")
plt.xlabel("Baseline (m)")
plt.ylabel("Spatial coherence (fs)")
plt.xlim(0,170)
plt.axhline(y=0, color="grey", linestyle="--")
...
def chunk_ana(star, telcombi):
    ...
    timestrings = np.loadtxt("g2_functions/{}/{}/ac_times.txt".format(star,pairstring))
    baselines   = np.loadtxt("g2_functions/{}/{}/baselines.txt".format(star,pairstring))
    dbaselines  = np.loadtxt("g2_functions/{}/{}/dbaselines.txt".format(star,pairstring))

    plt.figure("baselines")
    plt.errorbar(x=timestrings, y=baselines, yerr=dbaselines, marker="o", linestyle="", label=pairstring, color=combicolors[c1][c2])
    ...
    for i in range(0,len(chAs)):
        ...
        Int, dInt = uti.integral_fixed(popt_A, perr_A, sigma_A[c1][c2])
        ...
        plt.figure("blue")
        plt.errorbar(x=baseline, xerr=dbaseline, y=1e6*Int, yerr=1e6*dInt, marker="o", color=combicolors[c1][c2])
        
        xplotf, popt_B, perr_B = uti.fit_fixed(chB, x, -50, 50, mu_B[c1][c2], sigma_B[c1][c2])
        Int, dInt = uti.integral_fixed(popt_B, perr_B, sigma_B[c1][c2])
...
plt.figure("baselines"); plt.legend()
