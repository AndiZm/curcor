...
line = f.readline() #read lines
while source not in line: #solange lesen wie source nicht in zeile ist
    line = f.readline()
rad = float(line.split()[1] ) #aus while schleife raus wenn source in zeile und daten einlesen
...
print(realtime[0], ephem.Date(realtime[0]), sourcealt[0], ephem.Date(realtime[-1]), sourcealt[-1], ephem.Date(t_ob[0]), ephem.Date(t_ob[-1]))
...
plt.xlabel('baseline')
plt.savefig('%s/%s_%s_sc.pdf' %(source, source, date))
#np.savetxt("Namibia_April2022/, np.c_[realtime, sourcealt, baseline_3_4, baseline_1_4, separation])
np.savetxt("%s/%s_%s.txt" %(source, source, date), np.c_[year, month ,day ,hour, minute, sourcealt, bl_3_4, separation], fmt=' '.join(['%02d']*5 + ['%1.2f']*3), header = "year, month, day, hour, min, alt, baseline, separation moon")
np.savetxt("%s/%s_%s_data.txt" %(source, source, date), np.c_[t_ob, thour, tminute, salt, bl34, R34, S_bl34], fmt=' '.join(['%1.9f']*1 + ['%02d']*2 + ['%1.2f']*4), header = "realtime, hour, min, alt, baseline, rate, SC")
