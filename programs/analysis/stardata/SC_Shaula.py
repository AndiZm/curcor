...
while source in line:
    rad = int(line.split()[1] )
    dis = int(line.split()[2] )
    mag = int(line.split()[3] )
    line = f.readline()
f.close()
...
np.savetxt("%s/%s_%s_%s_data.txt" %(source, source, anfang, ende), np.c_[t_ob, thour, tminute, salt, bl34, R34, S_bl34], fmt=' '.join(['%1.9f']*1 + ['%02d']*2 + ['%1.2f']*4), header = "realtime, hour, min, alt, baseline, rate, SC")
