...
bigfigure = plt.figure(figsize=(6,4))
plt.style.use('dark_background')
...
plt.errorbar(x=5.43, y=1e6*int_auto, yerr=1e6*dint_auto, xerr=2.50, marker="o", color="white")
# cross correlations
...
if star != "Acrux":
    plt.fill_between(xplot, lower, upper, color="white", alpha=0.15)
...
#plt.fillxplot, _between(xplot, lower_A, upper_A, color="blue", alpha=0.3)
    ...
    plt.plot(xplot, uti.spatial_coherence(xplot,*popt_odr),   label="ODR fit", color="white", linewidth=2)
#plt.plot(xplot, uti.spatial_coherence(xplot,*poptavg_free),   label="Free parameters", color="red", linewidth=2, alpha=0.4)

if star != "Acrux":
    plt.text(100, 38, "Angular diameter: {:.2f} +/- {:.2f} mas".format(uti.rad2mas(popt_odr[1]),   uti.rad2mas(perr_odr[1])), color="white", fontsize=10)
...
plt.savefig("images/{}_sc_t.pdf".format(star), transparent=True)
plt.savefig("images/{}_sc_t.png".format(star), transparent=True)
plt.show()