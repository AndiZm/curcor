...
def execute(infiles, outfilepath, shifts, offset, packetlength, npackets, threshold):
	...
	for i in range (0, len(infiles)):
		...
		commands.append("python.exe ../pur_bin/curcor_int8_cpu.py -i " + infile + " -f " + resultpath + " -a " + str(threshold) + " -b " + str(threshold) + " -s " + str(shifts) + " -o " + str(offset) + " -l " + str(packetlength) + " -p " + str(npackets))		
